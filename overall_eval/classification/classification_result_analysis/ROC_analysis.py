...
def main():
    ...
    pairs = sorted(pairs, key=lambda p: aucs.get(p[1], -1.0), reverse=True)
    ...
    for i, (base, ft) in enumerate(pairs):
        color = palette[i % 10]
        if base in curves:
            fpr_b, tpr_b = curves[base]
            ax.plot(fpr_b, tpr_b, ls="--", lw=1.2, alpha=0.7, color=color,
                    label=f"{base} ({aucs.get(base, float('nan')):.3f})")
        if ft in curves:
            fpr_f, tpr_f = curves[ft]
            delta = aucs.get(ft, 0.0) - aucs.get(base, 0.0)
            ax.plot(fpr_f, tpr_f, ls="-", lw=2.0, color=color,
                    label=f"{ft} ({aucs.get(ft, float('nan')):.3f}, Δ{delta:+.3f})")

    # chance
    ax.plot([0, 1], [0, 1], ls="--", lw=0.8, color="gray", alpha=0.6, label="Chance")
    ax.set_title("ROC Full-corpus— all models (base dashed, ft solid)")
